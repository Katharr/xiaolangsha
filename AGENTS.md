...
第一轮 MVP 的核心目标是做出一个能在 10 分钟内完整体验的训练闭环：玩家能开始一局、完成关键行动、让 AI 自动推进其他玩家、看到结算、获得确定性评分和脚本复盘，并据此判断“小狼杀”的开发方向是否值得继续。

`specs/00` 到 `specs/07` 是第一轮 MVP 核心范围。`specs/08` 到 `specs/12` 是 MVP 后增强或安全边界：难度、AI 复盘、LLM 发言、扩展板子和 LLM 安全细则都不能抢在第一轮可玩闭环之前实现，除非当前 SPEC 明确切换到对应阶段。
