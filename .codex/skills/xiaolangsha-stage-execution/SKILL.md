...
- LLM 输出只能作为建议、文本或解释，不能修改 `GameState`、`Action`、`RoleScore`、`TimelineEvent`、胜负结果或规则状态。
- 测试必须证明隐藏身份和私密事件不会暴露给错误角色。
- 涉及视角、时间轴、AI、教练或 LLM 的阶段，必须新增或更新多视角信息泄露矩阵测试。
...
- 如果阶段涉及 LLM，说明 LLM payload 和输出越权测试是否覆盖。
- 新增或更新时间轴事件。
- 有意延后的项目。
