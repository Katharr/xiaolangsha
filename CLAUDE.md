...
4. **规则铁律（用户拍板，写测试一并验收）**：① **投票全程保密**——投票中零计票 UI，票向/票数只在 `vote_resolved` 一次性揭示（防跟票）；② **夜死不公布死法**——毒杀对局内一律显示「夜里出局」（`visibility.ts` 的 `maskNightDeathCause` 掩蔽 `vi.deadPlayers` 与公共 `player_died` payload），只在复盘揭示真相。③ **放逐达成胜负即终局**——跳过遗言与猎人开枪直接进复盘（`resolveVote` 放逐分支先 `checkWin`）；**女巫仅首夜可自救**（主流规则，非首夜自救一律非法，规则层/UI/脚本AI/提示词四处一致）。
...
- **前端重设计**（逻辑层不动，只改 UI 层）：阶段 0–5 + 主页 / 选身份页已完成。**当前布局 = 「圆桌剧场」单行三列**
  （左过程档案｜中大牌桌·座位铭牌+桌心铭牌｜右发言流+操作），StatusBar/Roster 已删除（信息上桌）；
  **投票全程保密**（投票中零计票 UI，一切票向/票数只在 vote_resolved 揭示——用户拍板的红线）。
  **阶段 5 复盘时间线已落地**：`deriveRoundArchives` 数据层（shared，四陷阱测试）+ `ReviewPanel` 时间线重构
  （`src/ui/review/`，props 不变）+ 侧边进度轨导航（≤1280 退化吸顶横条），预览基准 `preview/review-timeline-mockup.html`。
  **下一步 = 阶段 6 打磨 + 回归**。权威计划 `docs/FRONTEND-REDESIGN.md`。
- **阶段 4 落地后待手测**：投票→开票揭示动画→平票加赛→放逐全链、刷新恢复直出终态不重播、5 人板牌桌不崩。
- **阶段 5 落地后待手测**：复盘页真局验证——毒杀真相揭示、平票加赛链完整归档、时间轴交互（进度轨导航/点跳/键盘）。
- **AI 提示词**：代码已落地，剩**手测验证**（真人当预言家硬跳查杀，好人 AI 不再集体冤神；发言像人、不雷同）。
- **首日发言改进已落地待手测**：speech 任务按发言顺位分支（首位开场/中间位信息配额/末位归票，task.ts 就地从
  vi.speeches 推断顺位）、vote 加「带理由归票号召可并票」、人类侧新手发言提示卡（`SpeechCoachCard.tsx`，首日默认展开）。
  手测观察：AI 首轮发言有实在内容、末置位明确归票、首轮票收敛（最高票 ≥3）、提示卡按角色显示可折叠。
